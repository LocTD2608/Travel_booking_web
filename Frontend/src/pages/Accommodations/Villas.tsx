                ...
                <div className="w-full max-w-[1200px] px-4 flex items-center justify-between">
                    <div className="flex gap-10">
                        ...
                        <div>
                            ...
                            <div className="text-[15px] font-bold text-gray-900">{translateGuests('6 Adults, 1 Entire Villa')}</div>
                        </div>
                    </div>
                    <button 
                ...
                <div className="w-[280px] flex-shrink-0">
                    <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4 shadow-sm">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button className="text-travel-blue font-semibold text-sm hover:underline" onClick={() => { setPriceRange([2000000, 20000000]); setSortBy('popularity'); setSelectedTypes([]); }}>{t('search.reset', 'Reset')}</button>
                        </div>
                        <div className="mb-6">
                        ...
                        <div className="mb-6">
                            <h4 className="font-semibold text-[15px] mb-3">{t('search.villaType', 'Villa Type')}</h4>
                            {['Pool Villa', 'Beachfront', 'Mountain View', 'Family Friendly'].map(type => {
                                const typeDisplayName = type === 'Pool Villa' ? t('villa.type.pool', 'Biệt thự có hồ bơi')
                                    : type === 'Beachfront' ? t('villa.type.beachfront', 'Sát biển')
                                    : type === 'Mountain View' ? t('villa.type.mountain', 'Hướng núi')
                                    : type === 'Family Friendly' ? t('villa.type.family', 'Phù hợp gia đình')
                                    : type;
                                return (
                    ...
                    {!isAuthenticated && (
                        <div className="bg-green-700 text-white rounded-xl p-5 shadow-sm relative overflow-hidden">
                            ...
                            <h4 className="font-bold text-lg mb-2 relative z-10">{t('villa.exclusiveRates', 'Exclusive Villa Rates')}</h4>
                            <p className="text-sm text-green-100 mb-4 relative z-10">{t('villa.signInDesc', 'Sign in to see prices up to 30% lower on selected villas.')}</p>
