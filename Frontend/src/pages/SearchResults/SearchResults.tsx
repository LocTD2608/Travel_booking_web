        ...
        <div className="bg-white min-h-screen pb-10">
                ...
                <div className="w-full flex items-center pr-10" style={{ paddingLeft: '25%' }}>
                    <div className="flex-1 max-w-4xl border border-gray-300 rounded-lg p-3 flex shadow-sm bg-white gap-3 items-center">
                        ...
                        <RangePicker 
                            ...
                            format="DD/MM/YYYY"
                            placeholder={[t('detail.checkinPlaceholder', 'Check-in'), t('detail.checkoutPlaceholder', 'Check-out')]}
                        />
                        
                        <input type="text" list="guests-options" placeholder={t('search.guests', 'Guests')} className="flex-1 h-11 px-4 outline-none rounded-md bg-gray-50 border border-transparent focus:border-travel-blue focus:bg-white transition-all shadow-inner text-[15px] placeholder:text-gray-400 placeholder:font-medium" value={searchState.guests} onChange={(e) => setSearchState({...searchState, guests: e.target.value})} />
                        <datalist id="guests-options">
                            <option value={t('guests.1adult1room', '1 adult, 1 room')} />
                            <option value={t('guests.2adults1room', '2 adults, 1 room')} />
                            <option value={t('guests.2adults1child1room', '2 adults, 1 child, 1 room')} />
                            <option value={t('guests.3adults2rooms', '3 adults, 2 rooms')} />
                            <option value={t('guests.4adults2rooms', '4 adults, 2 rooms')} />
                            <option value={t('guests.family2rooms', 'Family, 2 rooms')} />
                            <option value={t('guests.group3rooms', 'Group, 3 rooms')} />
                        </datalist>
                        
                        <button className="bg-travel-blue text-white px-8 h-11 rounded-md font-bold hover:bg-blue-700 transition-colors shadow-sm">{t('hero.search.button', 'Search')}</button>
                    </div>
                </div>
            </div>
            ...
            <div className="px-4 md:px-10 lg:px-40 flex gap-6">
                ...
                <div className="w-1/3 border border-gray-200 rounded-lg p-5 h-fit sticky top-24">
                    <h3 className="font-bold text-lg mb-4">{t('results.filters', 'Filters')}</h3>
                    
                    {/* Price Filter */}
                    <div className="mb-6">
                        <div className="flex justify-between items-center mb-2">
                            <h4 className="font-semibold">{t('results.priceRange', 'Price Range')}</h4>
                        </div>
                    ...
                    {/* Star Rating */}
                    <div className="mb-6">
                        <h4 className="font-semibold mb-2">{t('results.starRating', 'Star Rating')}</h4>
                        {[5, 4, 3, 2, 1].map(star => (
                    ...
                    {/* Amenities */}
                    <div className="mb-6">
                        <h4 className="font-semibold mb-2">{t('results.amenities', 'Amenities')}</h4>
                ...
                <div className="w-2/3 flex flex-col gap-4">
                    ...
                    <div className="w-full flex justify-between items-center bg-gray-50 p-3 rounded-lg border border-gray-200 mb-4">
                        <span className="font-semibold text-gray-700 flex items-center gap-2">
                            ...
                            {t('results.sortBy', 'Sort results by:')}
                        </span>
                        <div className="flex gap-4">
                            ...
                            >
                                <option value="popularity">{t('results.sort.popularity', 'Popularity')}</option>
                                <option value="price_asc">{t('results.sort.price_asc', 'Price: Lowest First')}</option>
                                <option value="price_desc">{t('results.sort.price_desc', 'Price: Highest First')}</option>
                                <option value="rating_desc">{t('results.sort.rating_desc', 'Rating: Highest First')}</option>
                                <option value="rating_asc">{t('results.sort.rating_asc', 'Rating: Lowest First')}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center p-10 bg-white border border-gray-200 border-dashed rounded-lg text-gray-400 mt-4 h-64">
                        ...
                        <p className="font-medium text-gray-500">{t('results.noResults', 'No results to display')}</p>
                        <p className="text-sm">{t('results.noResultsDesc', 'Data will be populated here once backend is connected.')}</p>
                    </div>
                </div>
            </div>
        </div>
