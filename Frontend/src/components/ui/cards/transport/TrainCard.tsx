...
export const TrainCard: React.FC<{ train: TrainCardProps; onSelect?: () => void }> = ({ train, onSelect }) => {
    const { t } = useLanguage();
    const classLabel = train.class === 'Soft Bed' ? t('transport.softBed', 'Soft Bed') :
                        train.class === 'Hard Bed' ? t('transport.hardBed', 'Hard Bed') :
                        train.class === 'Soft Seat' ? t('transport.softSeat', 'Soft Seat') :
                        train.class === 'Hard Seat' ? t('transport.hardSeat', 'Hard Seat') :
                        train.class;
            ...
            <div className="flex justify-between items-center mb-4 pl-2">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-gray-500">{t('transport.trainNumber', 'Train {num}').replace('{num}', train.trainNumber)}</div>
                    </div>
                </div>
            ...
            <div className="flex justify-between items-center pl-2">
                ...
                <div className="text-right border-l border-gray-100 pl-6 min-w-[200px]">
                    ...
                    <button className="bg-[#FF5E1F] text-white px-6 py-2 rounded-lg font-bold hover:bg-[#E64B0E] transition-colors hover-scale w-full shadow-sm" onClick={onSelect ? (e) => { e.stopPropagation(); onSelect(); } : undefined}>
                        {t('transport.selectWaitlist', 'Select Waitlist')}
                    </button>
                </div>
            </div>
        </div>
